    ...
    if(status != "")
    {
        ...
        for (i = 0; i <objects.length; i++) {
            ...
            if(objects[i].label == objectName)
            {
                ...
                document.getElementByld("objectStatus").innerHTML = objectName + " Encontrado";
                synth = window.speechStyntheis;
                utterThis = new SpeechSynthesisUtterance(objectName + " Encontrado");
                synth.speak(utterThis);
            }
            else
            {
                document.getElementByld("objectStatus").innerHTML = objectName + " Não Encontrado";
            }
        }
    }
}